...
using OldTVGL.IOFunctions;

namespace OldTVGL.Voxelization
{
    /// <inheritdoc />
    /// <summary>
    ...
    public partial class VoxelizedSolid : Solid, IEnumerable<int[]>
    {
        ...
        private void FillInFromTessellation(TessellatedSolid ts)
        {
            ...
            //for (var k = 0; k < numVoxelsZ; k++)
            {
                ...
                if (loops.Any())
                {
                    var intersections =PolygonOperations.AllPolygonIntersectionPointsAlongY(loops.Select(p=>new PolygonClass(p)), yBegin, numVoxelsY, VoxelSideLength, out var yStartIndex);
