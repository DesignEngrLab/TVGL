...
namespace TVGLUnitTestsAndBenchmarking
{
    public static partial class TVGL3Dto2DTests
    {
        ...
        public static void TestXSectionAndMonotoneTriangulate()
        {
            ...
            //var fileNames = dir.GetFiles("failedTri*").OrderBy(x => r.NextDouble()).ToArray();
            var fileNames = dir.GetFiles("failedTri*").ToArray();
            for (var i = 0; i < fileNames.Length; i++)
            {
                var filename = fileNames[i].FullName;
                var name = fileNames[i].Name;
                Console.WriteLine("Attempting: " + filename);
                IO.Open(filename, out Polygon p);
                TestTriangulate(p);
                continue;
                ...
                foreach (var monopoly in p.CreateXMonotonePolygons())
                {
