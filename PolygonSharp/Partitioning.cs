        ...
        public static MonotonicityChange GetMonotonicityChange(this Vertex2D vertex)
        {
            ...
            ****/
            ...
            if (xPrev == 0 && xNext == 0) // then line is vertical
                return (MathF.Sign(yPrev) == MathF.Sign(yNext)) ? MonotonicityChange.Neither : MonotonicityChange.Y;
            ...
            if (yPrev == 0 && yNext == 0) // then line is horizontal
                return (MathF.Sign(xPrev) == MathF.Sign(xNext)) ? MonotonicityChange.Neither : MonotonicityChange.X;
                            ...
                            // the condition before the last). it's possible that yNext is zero, but it doesn't affect the approach
            {
                ...
                xChangesDir = MathF.Sign(xPrev) != MathF.Sign(xNext);
            }
            else if (xNext == 0)
            {
                do
            ...
            if (yPrev == 0)
            {
                ...
                yChangesDir = (MathF.Sign(yPrev) != MathF.Sign(yNext));
            }
            else if (yNext == 0)
            {
                do
