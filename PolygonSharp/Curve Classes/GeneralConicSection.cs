...
namespace PolygonSharp
{
    ...
    public struct GeneralConicSection : ICurve
    {
        ...
        private const float conicTolerance = 1e-4f;

        public PrimitiveCurveType CurveType { get; private set; }

        public GeneralConicSection(float a, float b, float c, float d, float e, bool constantIsZero)
        {
            var max = (new[] { MathF.Abs(a), MathF.Abs(b), MathF.Abs(c), MathF.Abs(d), MathF.Abs(e) }).Max();
            A = MathF.Abs(a / max) < conicTolerance ? 0 : a;
            B = MathF.Abs(b / max) < conicTolerance ? 0 : b;
            C = MathF.Abs(c / max) < conicTolerance ? 0 : c;
            D = MathF.Abs(d / max) < conicTolerance ? 0 : d;
            E = MathF.Abs(e / max) < conicTolerance ? 0 : e;
        ...
        public float CalculateAtPoint(Vector2 point)
        {
            float x = point.X;
            float y = point.Y;
        ...
        internal bool DefineCircleFromTerms(out Circle circle1)
        {
            ...
            var xCenter = -rSqMinusXSqMinusYSq * D/2;
            var yCenter = - rSqMinusXSqMinusYSq * E/2;
            var radiusSq = rSqMinusXSqMinusYSq + (xCenter * xCenter) + (yCenter * yCenter);
            circle1 = new Circle(new Vector2(xCenter, yCenter), MathF.Abs(radiusSq));
            return true;
        }
        /// <summary>
        ...
        public static double DistancePointToConic<T>(GeneralConicSection conic, Vector2 point, out Vector2 pointOnCurve)
        {
            ...
            pointOnCurve = Constants.NullVector;
