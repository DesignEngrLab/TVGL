...
namespace PolygonSharp
{
    ...
    public readonly struct Circle : ICurve
    {
        ...
        [JsonConstructor]
        public Circle(Vector2 center, float radiusSquared)
        {
            Center = center;
            RadiusSquared = radiusSquared;
            Radius = MathF.Sqrt(radiusSquared);
            Area = MathF.PI * radiusSquared;
        ...
        {
            var diff = new Vector2((float)(point.X - Center.X), (float)(point.Y - Center.Y));
            var error = MathF.Sqrt(diff.Dot(diff)) - Radius;
            return error * error;
        }
        ...
        public static bool CreateFromPoints<T>(IEnumerable<T> points, out Circle curve, out double error) where T : IVertex2D
        {
            ...
            var center = new Vector2((float)xc, (float)yc);
            ...
            #endregion
            curve = new Circle(center, (float)radiusSquared);
            error = 0.0;
            foreach (var p in points)
        ...
        public List<Vector2> CreatePath(int numPoints)
        {
            ...
            var increment = MathF.PI * 2 / numPoints;
            var path = new List<Vector2>();
            for (float theta = 0f; theta < MathF.PI * 2; theta += increment)
            {
                path.Add(new Vector2(Radius * MathF.Cos(theta) + Center.X, Radius * MathF.Sin(theta) + Center.Y));
            }
