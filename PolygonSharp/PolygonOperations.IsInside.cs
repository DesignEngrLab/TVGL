    ...
    public static partial class PolygonOperations
    {
        ...
        public static bool IsPointInsidePolygon(this Polygon polygon, bool onlyTopPolygon, Vector2 pointInQuestion)
        {
            ...
            foreach (var subPolygon in polygon.AllPolygons.Skip(1))
            {
                ...
                {
                    var absArea = MathF.Abs(subPolygon.PathArea);
            ...
            double perpendicularDistanceToLine, int numSteps, double stepSize, out int firstIntersectingIndex)
        {
            ...
            firstIntersectingIndex = (int)MathF.Ceiling((nextDistance - perpendicularDistanceToLine) / stepSize);
            var pointIndex = 0;
            for (int i = firstIntersectingIndex; i < numSteps; i++)
            {
                ...
                if (needToOffset)
                    d += MathF.Min(stepSize, sortedPoints[pointIndex].Coordinates.Dot(lineNormalDirection) - d) / 10.0;
              ...
              int numSteps, double stepSize, out int firstIntersectingIndex)
        {
            ...
            firstIntersectingIndex = (int)MathF.Ceiling((nextDistance - startingXValue) / stepSize);
            var pointIndex = 0;
            for (int i = firstIntersectingIndex; i < numSteps; i++)
            {
                ...
                if (needToOffset)
                    x += MathF.Min(stepSize, sortedPoints[pointIndex + 1].X - x) / 10.0;
                var numIntersects = currentLines.Count;
            ...
            double startingYValue, int numSteps, double stepSize, out int firstIntersectingIndex)
        {
            ...
            firstIntersectingIndex = (int)MathF.Ceiling((nextDistance - startingYValue) / stepSize);
            var pointIndex = 0;
            for (int i = firstIntersectingIndex; i < numSteps; i++)
            {
                ...
                if (needToOffset)
                    y += MathF.Min(stepSize, sortedPoints[pointIndex].Y - y) / 10.0;
        ...
        {
            var numSigDigs = MathF.Min(subPolygonA.NumSigDigits, subPolygonB.NumSigDigits);
            ...
            {
                var aXMin = needToRoundA ? MathF.Round(aLines[aIndex].XMin, numSigDigs) : aLines[aIndex].XMin;
                var bXMin = needToRoundB ? MathF.Round(bLines[bIndex].XMin, numSigDigs) : bLines[bIndex].XMin;
                if (aXMin < bXMin) // if the next A-line is lower compare it to all B-lines
                {
                    var aLine = aLines[aIndex++];
                    var aXMax = needToRoundA ? MathF.Round(aLine.XMax, numSigDigs) : aLine.XMax;
                    ...
                    while (aXMax >= bXMin)
                    {
                        ...
                        bXMin = needToRoundB ? MathF.Round(bLines[localBIndex].XMin, numSigDigs) : bLines[localBIndex].XMin;
                    }
                }
                else
                {
                    var bLine = bLines[bIndex++];
                    var bXMax = needToRoundB ? MathF.Round(bLine.XMax, numSigDigs) : bLine.XMax;
                    ...
                    while (bXMax >= aXMin)
                    {
                        ...
                        aXMin = needToRoundA ? MathF.Round(aLines[localAIndex].XMin, numSigDigs) : aLines[localAIndex].XMin;
                    }
                }
            }
            ...
            var equalTolerance = MathF.Pow(10, -numSigDigs);
            ...
            bool needToRoundA, bool needToRoundB)
        {
            ...
            var aFrom = needToRoundA ? new Vector2(MathF.Round(lineA.FromPoint.X, numSigDigs), MathF.Round(lineA.FromPoint.Y, numSigDigs))
                : lineA.FromPoint.Coordinates;
            var aTo = needToRoundA ? new Vector2(MathF.Round(lineA.ToPoint.X, numSigDigs), MathF.Round(lineA.ToPoint.Y, numSigDigs))
                : lineA.ToPoint.Coordinates;
            if (needToRoundA && aTo == aFrom) return false;
            var aVector = aTo - aFrom;
            var bFrom = needToRoundB ? new Vector2(MathF.Round(lineB.FromPoint.X, numSigDigs), MathF.Round(lineB.FromPoint.Y, numSigDigs))
                : lineB.FromPoint.Coordinates;
            var bTo = needToRoundB ? new Vector2(MathF.Round(lineB.ToPoint.X, numSigDigs), MathF.Round(lineB.ToPoint.Y, numSigDigs))
            ...
            if (MathF.Max(aFrom.Y, aTo.Y) < MathF.Min(bFrom.Y, bTo.Y) || MathF.Max(bFrom.Y, bTo.Y) < MathF.Min(aFrom.Y, aTo.Y))
                // the two lines do not touch since their bounding boxes do not overlap
                return false;
            // okay, so bounding boxes DO overlap
            var intersectionCoordinates = Constants.NullVector;
            ...
            else
            {
                ...
                else
                {
                    ...
                    var aIntersection = new Vector2(
                            MathF.Round(aFrom.X + t_1 * aVector.X, numSigDigs),
                            MathF.Round(aFrom.Y + t_1 * aVector.Y, numSigDigs));
                    var bIntersection = new Vector2(
                            MathF.Round(bFrom.X + t_2 * bVector.X, numSigDigs),
                            MathF.Round(bFrom.Y + t_2 * bVector.Y, numSigDigs));
                    ...
                    else
                    {
                        intersectionCoordinates = new Vector2(
                            MathF.Round((aFrom.X + t_1 * aVector.X + bFrom.X + t_2 * bVector.X) / 2, numSigDigs),
                            MathF.Round((aFrom.Y + t_1 * aVector.Y + bFrom.Y + t_2 * bVector.Y) / 2, numSigDigs));
            ...
            in Vector2 aVector, in Vector2 bVector, int numSigDigs, bool needToRoundA, bool needToRoundB, in WhereIsIntersection where, double lineACrossLineB)
        {
            ...
            { //then lineB and prevB are the same
                ...
                if (needToRoundA)
                {
                    previousAVector = new Vector2(
                        MathF.Round(previousALine.ToPoint.X, numSigDigs) - MathF.Round(previousALine.FromPoint.X, numSigDigs),
                        MathF.Round(previousALine.ToPoint.Y, numSigDigs) - MathF.Round(previousALine.FromPoint.Y, numSigDigs)
                        );
                }
            ...
            { //then lineA and prevA are the same
                ...
                if (needToRoundB)
                {
                    previousBVector = new Vector2(
                        MathF.Round(previousBLine.ToPoint.X - previousBLine.FromPoint.X, numSigDigs),
                        MathF.Round(previousBLine.ToPoint.Y - previousBLine.FromPoint.Y, numSigDigs)
                        );
                }
            ...
            else // then where == BothLinesStart
            {
                ...
                {
                    previousAVector = new Vector2(
                        MathF.Round(previousALine.ToPoint.X - previousALine.FromPoint.X, numSigDigs),
                        MathF.Round(previousALine.ToPoint.Y - previousALine.FromPoint.Y, numSigDigs)
                        );
                }
                else previousAVector = previousALine.Vector;
                var previousBLine = edgeB.FromPoint.EndLine;
                if (needToRoundB)
                {
                    ...
                    //previousBVector = new Vector2(
                    //    MathF.Round(previousBLine.ToPoint.X, numSigDigs) - MathF.Round(previousBLine.FromPoint.X, numSigDigs),
                    //    MathF.Round(previousBLine.ToPoint.Y, numSigDigs) - MathF.Round(previousBLine.FromPoint.Y, numSigDigs)
                    //    );
                }
                else previousBVector = previousBLine.Vector;
            ...
            if (lineACrossLineB.IsNegligible() || MathF.Abs(lineACrossLineB) / (aVector.Length() * bVector.Length()) < unitCrossIsParallel)
                lineACrossLineB = 0;
            if (prevACrossPrevB.IsNegligible() || MathF.Abs(prevACrossPrevB) / (previousAVector.Length() * previousBVector.Length()) < unitCrossIsParallel)
                prevACrossPrevB = 0;
            if (lineACrossPrevB.IsNegligible() || MathF.Abs(lineACrossPrevB) / (aVector.Length() * previousBVector.Length()) < unitCrossIsParallel)
                lineACrossPrevB = 0;
            if (prevACrossLineB.IsNegligible() || MathF.Abs(prevACrossLineB) / (previousAVector.Length() * bVector.Length()) < unitCrossIsParallel)
                prevACrossLineB = 0;
