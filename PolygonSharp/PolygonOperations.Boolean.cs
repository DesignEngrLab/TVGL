    ...
    public static partial class PolygonOperations
    {
        ...
        private static bool Compare(List<Polygon> tvglResult, List<Polygon> clipperResult, string operationString, TimeSpan clipTime, TimeSpan tvglTime)
        {
            lock (sw)
            {
                ...
                if (
                    //((double)MathF.Abs(numPolygonsTVGL - numPolygonsClipper) / (numPolygonsTVGL + numPolygonsClipper + tolerance)).IsNegligible(tolerance) &&
                    //((double)MathF.Abs(vertsTVGL - vertsClipper) / (vertsTVGL + vertsClipper + tolerance)).IsNegligible(tolerance) &&
                    (MathF.Abs(areaTVGL - areaClipper) / (areaTVGL + areaClipper + tolerance)).IsNegligible(tolerance)  //&&
                    || (areaTVGL<0.15 && areaClipper<0.15)
                    //(MathF.Abs(perimeterTVGL - perimeterClipper) / MathF.Abs(perimeterTVGL + perimeterClipper + tolerance)).IsNegligible(tolerance) &&
                ...
                else
                {
                    ...
                    if (perimeterClipper - perimeterTVGL > 0 && MathF.Round(perimeterClipper - perimeterTVGL) % 2 == 0)
