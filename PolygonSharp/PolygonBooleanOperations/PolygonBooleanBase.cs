    ...
    internal abstract class PolygonBooleanBase
    {
            ...
            double minimumArea = double.NaN)
        {
            if (double.IsNaN(minimumArea))
                minimumArea = (MathF.Abs(polygonA.PathArea) + MathF.Abs(polygonB.PathArea))
                    * MathF.Pow(10, -(MathF.Max(polygonA.NumSigDigits, polygonB.NumSigDigits)));
...

        {
            ...
            do
            {
                ...
                // out of the loop. The intersection is identified here, but processed above
                {
                    ...
                    intersectionCoordinates = Constants.NullVector; // this is set to null because its value is used in ClosestNextIntersectionOnThisEdge
