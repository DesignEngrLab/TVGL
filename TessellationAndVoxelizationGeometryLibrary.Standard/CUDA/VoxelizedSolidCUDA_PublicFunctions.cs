...
namespace TVGL.CUDA
{
    ...
    public partial class VoxelizedSolidCUDA
    {
        public int[][] GetNeighbors(int i, int j, int k)
        {
            ...
            if (i + 1 != VoxelsPerSide[0] && Voxels[i + 1, j, k] == 1) neighbors[1] = new[] { i + 1, j, k };
            if (j + 1 != VoxelsPerSide[1] && Voxels[i, j + 1, k] == 1) neighbors[3] = new[] { i, j + 1, k };
            if (k + 1 != VoxelsPerSide[2] && Voxels[i, j, k + 1] == 1) neighbors[5] = new[] { i, j, k + 1 };
            if (i != 0 && Voxels[i - 1, j, k] == 1) neighbors[0] = new[] { i - 1, j, k };
            if (j != 0 && Voxels[i, j - 1, k] == 1) neighbors[2] = new[] { i, j - 1, k };
            if (k != 0 && Voxels[i, j, k - 1] == 1) neighbors[4] = new[] { i, j, k - 1 };
            return neighbors;
        }
        ...
        {
            Count = Voxels.Cast<byte>().Count(vox => vox == 1);
        }
        ...
        private void SetSurfaceArea()
        {
            ...
            {
                if (k == 1 && Voxels[i, j, k - 1] == 1)
                    sa++;
                if (Voxels[i, j, k] != Voxels[i, j, k - 1])
                    sa++;
                if (k == VoxelsPerSide[2] - 1 && Voxels[i, j, k] == 1)
                    sa++;
            }
            for (var j = 0; j < VoxelsPerSide[1]; j++)
            ...
            for (var i = 1; i < VoxelsPerSide[0]; i++)
            {
                ...
                if (Voxels[i, j, k] != Voxels[i - 1, j, k])
                    sa++;
                if (i == VoxelsPerSide[0] - 1 && Voxels[i, j, k] == 1)
                    sa++;
            }
            for (var k = 0; k < VoxelsPerSide[2]; k++)
            for (var i = 0; i < VoxelsPerSide[0]; i++)
            for (var j = 1; j < VoxelsPerSide[1]; j++)
            {
                ...
                if (Voxels[i, j, k] != Voxels[i, j - 1, k])
                    sa++;
                if (j == VoxelsPerSide[1] - 1 && Voxels[i, j, k] == 1)
                    sa++;
            }
