...
namespace TVGL.Voxelization
{
    ...
    public partial class VoxelizedSolid
    {
        ...
        public VoxelizedSolid(TessellatedSolid ts, VoxelDiscretization voxelDiscretization, bool onlyDefineBoundary = false,
            double[][] bounds = null, bool useAlternateFaceVoxellizer = false) : base(ts.Units, ts.Name, "", ts.Comments, ts.Language)
        {
            ...
            if(useAlternateFaceVoxellizer) makeVoxelForFacesAndEdgesAlternate(ts);
            else makeVoxelsForFacesAndEdges(ts);
            ...
            IEnumerable<long> voxelsInQuestion)
        {
            ...
            foreach (var voxel in voxels)
            {
                var bottomCoordinate = new[]
                {
                    (double) ((voxel.ID & Constants.maskAllButX) >> 40),
                    (double) ((voxel.ID & Constants.maskAllButY) >> 20),
                    (double) (voxel.ID & Constants.maskAllButZ)
                };
                bottomCoordinate = bottomCoordinate.multiply(VoxelSideLengths[4]).add(Offset);
        ...
        private void MakeInteriorVoxelsAlongLine(HashSet<Voxel_Level1_Class> negativeFaceVoxels,
            HashSet<Voxel_Level1_Class> positiveFaceVoxels, SortedSet<Voxel_Level1_Class> all, int directionIndex)
        {
            var direction = new[] { 0.0, 0.0, 0.0 };
            ...
            var startIndex = sortedVoxelsInRow.First().CoordinateIndices[directionIndex];
            var finalIndex = sortedVoxelsInRow.Last().CoordinateIndices[directionIndex];
            ...
            while (sortedVoxelsInRow.Any())
            {
                ...
                var currentIndex = currentVoxel.CoordinateIndices[directionIndex];
                ...
                if (sortedVoxelsInRow.Peek().CoordinateIndices[directionIndex] - currentIndex == 1) continue;
                ...
                realCoordinate[directionIndex] += VoxelSideLengths[voxelLevel];
                ...
                {
                    var dot = face.Normal[directionIndex];
                ...
                if (isInside == (intersectionPoints.Count % 2 == 0))
                {
                    ...
                    nextVoxelCoord[directionIndex]++;
                    var nextVoxelIndex = MakeVoxelID1(nextVoxelCoord[0], nextVoxelCoord[1], nextVoxelCoord[2]);
