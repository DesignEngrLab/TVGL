...
namespace TVGL
{
    ...
    public static class SilhouetteDebugger
    {
        ...
        double minPathAreaToConsider = 0.0, double depthOfPart = 0.0)
        {
            ...
            //Offset out and then perform a quick check for overhang polygons.
            ...
            //Presenter.ShowAndHang(offsetPolygons);
            var significantSolution = PolygonOperations.OffsetMiter(offsetPolygons, -scale);
            //significantSolution = PolygonOperations.Union(significantSolution, true, PolygonFillType.Positive);
        ...
        private static IEnumerable<List<Point>> GetSurfacePaths(HashSet<PolygonalFace> surface, double[] normal,
            double minAreaToConsider, TessellatedSolid originalSolid, Dictionary<int, List<Point>> projectedFacePolygons)
        {
            ...
            if (surfacePaths.Sum(p => p.Count) > 10)
            {
