...
namespace TVGL.IOFunctions
{
    internal class ShellFileData : IO
    {
        ...
        internal static List<TessellatedSolid> Open(Stream s, bool inParallel = true)
        {
            ...
            ShellFileData shellData;
            // Read in ASCII format
            if (PLYFileData.TryReadAscii(s, out shellData))
            ...
            {
                new TessellatedSolid(shellData.Name, shellData.Vertices, shellData.FaceToVertexIndices,
                ...
                switch (id)
                {
                    case "shell":
                        if (string.IsNullOrWhiteSpace(values))
                            shellSolid.Name = defaultName + (++solidNum);
                        shellSolid.Name = values.Trim(' ');
                        break;
                    case "material":
                        var listOfValues = values.Split(' ');
                        shellSolid.Material = values[0].Trim(' ');
                        break;
                    case "facet":
                        shellSolid.ReadFacet(reader, values);
                        break;
                    case "endshell":
                        shellData.Add(shellSolid);
                        shellSolid = new STLFileData();
                        break;
                }
            }
            return true;
