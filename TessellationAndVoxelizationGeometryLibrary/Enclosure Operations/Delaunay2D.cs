    ...
    public class Delaunay2D
    {
        ...
        public static bool Create<T>(List<T> points, out Delaunay2D delaunay2D) where T : IVector2D
        {
            ...
            for (int i = 0; i < points.Count(); i++)
            {
                ...
                vertices.Add(new Vertex(points[i].X, points[i].Y, points[i].X * points[i].X + points[i].Y * points[i].Y, vIndex));
            }

            if (ConvexHull3D.Create(vertices, out var convexHull, true, false))
            {
                ...
                {
                    for (int i = 0; i < points.Count(); i++)
                        vertices[i].Coordinates = new Vector3(vertices[i].X, vertices[i].Y, ((IVector3D)points[i]).Z);
                }
                else
                    foreach (var v in vertices)
                        v.Coordinates = new Vector3(v.X, v.Y, 0);
