...
namespace TVGL
{
    ...
    public static class PrimitiveSurfaceExtensions
    {
        ...
        public static Polygon GetAs3DPolygon(this PrimitiveSurface surface)
        {
            var polygons = new List<Polygon>();
            var vertexLoops = surface.OuterEdges.MakeEdgePaths(true,
                new EdgePathLoopsAroundInputFaces(surface.Faces)).Select(ep => ep.GetVertices().ToList());
        ...
        /*
        public static List<EdgePathLoopsAroundInputFaces> ConvertOuterEdgesToEdgePaths(this PrimitiveSurface ThisSurface, out Dictionary<Vector3, Queue<Edge>> edgeDictionary)
        {
            const double TOLERANCE = 1e-2;
            ThisSurface.SetFacesAndVertices(ThisSurface.Faces);
            MiscFunctions.DefineInnerOuterEdges(ThisSurface.Faces,
              out HashSet<Edge> innerEdges, out HashSet<Edge> outerEdges);
            var edgesByVertex = new Dictionary<Vector3, List<Edge>>();

            foreach (var e in outerEdges)
            {
                if (e.From == null || e.To == null) continue;
                var kFrom = FindOrCreateKey(edgesByVertex.Keys, e.From.Coordinates, TOLERANCE);
                if (!edgesByVertex.ContainsKey(kFrom)) edgesByVertex[kFrom] = new List<Edge>();
                edgesByVertex[kFrom].Add(e);

                var kTo = FindOrCreateKey(edgesByVertex.Keys, e.To.Coordinates, TOLERANCE);
                if (!edgesByVertex.ContainsKey(kTo)) edgesByVertex[kTo] = new List<Edge>();
                edgesByVertex[kTo].Add(e);
            }
            edgeDictionary = edgesByVertex.ToDictionary(kvp => kvp.Key, kvp => new Queue<Edge>(kvp.Value));

            var result = new List<Polygon3D>();
            var usedEdges = new HashSet<Edge>();
            Vector3 OtherEnd(Edge e, Vector3 v)
            {
                if (AreClose(e.From.Coordinates, v, TOLERANCE)) return e.To.Coordinates;
                return e.From.Coordinates;
            }
            while (true)
            {
                var seed = outerEdges.FirstOrDefault(e => !usedEdges.Contains(e));
                if (seed == null) break;
                usedEdges.Add(seed);
                var path = new List<Vector3>();
                var startVertex = seed.From.Coordinates;
                var currentVertex = seed.To.Coordinates;
                ...
                while (true)
                {
                    if (AreClose(currentVertex, startVertex, TOLERANCE))
                    {
                        if (path.Count > 4) result.Add(new Polygon3D(path, true));
                        break;
                    }
                    var vKey = FindOrCreateKey(edgesByVertex.Keys, currentVertex, TOLERANCE);
                    if (!edgesByVertex.TryGetValue(vKey, out var incident) || incident.Count == 0)
                    {
                        if (path.Count > 4) result.Add(new Polygon3D(path, false));
                        break;
                    }
                    var curDir = (currentVertex - previousVertex).Normalize();

#pragma warning disable CS8600 // Converting null literal or possible null value to non-nullable type.
                    Edge next = incident.Where(e => !usedEdges.Contains(e)).OrderByDescending(e =>
                    {
                        var nextVertex = OtherEnd(e, currentVertex);
                        var candDir = (nextVertex - currentVertex).Normalize();
                        return curDir.Dot(candDir);
                    }).FirstOrDefault();
#pragma warning restore CS8600 // Converting null literal or possible null value to non-nullable type.
                    ...
                    currentVertex = OtherEnd(next, currentVertex);
                    path.Add(currentVertex);
                }
            }
        ...
        public static List<Polygon3D> BuildPolygonsFromUnknownRegionSurfaceFirst(PrimitiveSurface ThisSurface, out Dictionary<Vector3, Queue<Edge>> edgeDictionary)
        {
            MiscFunctions.DefineInnerOuterEdges(ThisSurface.Faces, out HashSet<Edge> innerEdges, out HashSet<Edge> outerEdges);
            var OuterEdges = ThisSurface.OuterEdges
              .Where(e =>
                ((((e.From.Faces.Count - 1) * 2) >= e.From.Edges.Count) ||
                 (((e.To.Faces.Count - 1) * 2) >= e.To.Edges.Count)))
              .ToHashSet();
            edgeDictionary = outerEdges
              .GroupBy(e => e.From.Coordinates)
              .ToDictionary(g => g.Key, g => new Queue<Edge>(g));

            var PolygonEdges = new List<Polygon3D>();
            var PolygonPath = new List<Vector3>();

            while (edgeDictionary.Count > 0)
            {
                var StartOfSegmant = edgeDictionary.Keys.First();
                var edgesAtStart = edgeDictionary[StartOfSegmant];
                var currentEdge = edgesAtStart.Dequeue();
                if (edgesAtStart.Count == 0)
                    edgeDictionary.Remove(StartOfSegmant);

                PolygonPath = new List<Vector3>();
                var startVertex = currentEdge.From.Coordinates;
                var currentVertex = currentEdge.To.Coordinates;
                PolygonPath.Add(startVertex);
                PolygonPath.Add(currentVertex);

                while (true)
                {
                    if (currentVertex == startVertex)
                    {
                        if (PolygonPath.Count > 2)
                            PolygonEdges.Add(new Polygon3D(PolygonPath, true));
                        break;
                    }
                    if (!edgeDictionary.TryGetValue(currentVertex, out var nextEdges) || nextEdges.Count == 0)
                    {
                        PolygonEdges.Add(new Polygon3D(PolygonPath, false));
                        break;
                    }
                    var nextEdge = nextEdges.Dequeue();
                    if (nextEdges.Count == 0)
                        edgeDictionary.Remove(currentVertex);

                    currentVertex = nextEdge.To.Coordinates;
                    PolygonPath.Add(currentVertex);
                }
            }
            return PolygonEdges;
