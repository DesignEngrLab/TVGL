...
namespace TVGL
{
    ...
    public class Plane : PrimitiveSurface
    {
            ...
            out Vector3 normal)
        {
            ...
            if (absoluteDiff.X.IsNegligible(Constants.BaseTolerance)) normal = Vector3.UnitX;
            else if (absoluteDiff.Y.IsNegligible(Constants.BaseTolerance)) normal = Vector3.UnitY;
            else if (absoluteDiff.Z.IsNegligible(Constants.BaseTolerance)) normal = Vector3.UnitZ;
            else
            {
                normal = Vector3.Null;
                distanceToPlane = double.NaN;
