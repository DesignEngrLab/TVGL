...
namespace TVGL
{
    ...
    public static class TriangulatePolygon
    {
                    ...
                    if (ignoreNegativeSpace)
                    {
                        ...
                        foreach (var positiveLoop1 in orderedLoops)
                        {
                            ...
                            foreach (var positiveLoop2 in orderedLoops)
                            {
                                ...
                                if (!IsPointInsidePolygon(points2D[positiveLoop2.First().LoopID].ToList(), positiveLoop1.First().Point)) continue;
                                isInside = true;
                                break;
                            }
        ...
        /// <param name="isDirectionalityKnown"></param>
        /// <returns></returns>
        public static List<List<Vector2>> OrderLoops2D(IList<List<Vector2>> points2D, ref bool[] isPositive, bool isDirectionalityKnown = false)
        {
            ...
            var sortedLoops = new List<List<Node>>();
            var pointCount = 0;
            var outputLoops = new List<List<Vector2>>(points2D);
            ...
            var theta = values[attempts - 1];
            var points2Dtemp = points2D;
            points2D = new List<List<Vector2>>();
            foreach (var loop in points2Dtemp)
            {
                var newLoop = new List<Vector2>();
                var pHighest = double.NegativeInfinity;
            ...
            ////Convert the groups from lists of ints to points
            //var pointLoopGroups = groups.Select(@group => @group.Select(k => orderedPointLoops[k]).ToList()).ToList();

            //return pointLoopGroups;
        }
        ...
        {
            var angle = InteriorAngleBetweenEdgesInCCWList(a.Point, b.Point, c.Point);
        ...
        internal static List<Vector2[]> Triangulate(MonotonePolygon monotonePolygon)
        {
            ...
            for (var i = 2; i < sortedNodes.Count; i++)
            {
                ...
                {
                    triangles.Add(new[] { node.Point, scan[0].Point, scan[1].Point });
                    continue;
                }
                ...
                else
                {
                    ...
                    while (scan.Count > 1 && exitBool == false)
                    {
                        ...
                        var angle = (node.IsRightChain)
                            ? InteriorAngleBetweenEdgesInCCWList(node.Point, scan.Last().Point, scan[scan.Count - 2].Point)
                            : ExteriorAngleBetweenEdgesInCCWList(node.Point, scan.Last().Point, scan[scan.Count - 2].Point);
                        ...
                        triangles.Add(new[] { scan[scan.Count - 2].Point, scan.Last().Point, node.Point });
            ...
            foreach (var triangle in triangles)
            {
                ...
                var area = Math.Abs(edge1.Cross(edge2)) / 2;
        ...
        private static bool IsPointInsidePolygon(this List<Vector2> polygon, Vector2 pointInQuestion)
        {
