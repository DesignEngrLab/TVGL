...
namespace TVGL.Polygon_Operations.Clipper
{
	...
	public struct PathI : Path<PointI>, IList<PointI>
	{
		...
		public void RemoveAt(int i) => data.RemoveAt(i);
		public void Resize(int n) => data = new List<PointI>(n);
		IEnumerator IEnumerable.GetEnumerator() => GetEnumerator();
		public void Rotate(PointD center, double angle_rad) 
		{
			double cos_a = Math.Cos(angle_rad);
...
//void Trim(bool is_closed_path, T min_length);
  ...
  public void AppendPointsScale(const Path<T> other, double scale)
	{
	...
	{
		std::transform(other.data.begin(), other.data.end(), std::back_inserter(data),[scale](Point < T2 > p) { 
			return Point<T>(p * scale); 
		});
		//	for (const auto &p : other.data)
