    ...
    public static partial class PolygonOperations
    {
        ...
        private static bool Compare(List<Polygon> tvglResult, List<Polygon> clipperResult, string operationString, TimeSpan clipTime, TimeSpan tvglTime)
        {
            lock (sw)
            {
                ...
                {
                    Global.Logger.LogInformation("***** " + operationString + " matches", 4);
                    Global.Logger.LogInformation("clipper time = " + clipTime + "; tvgl time = " + tvglTime, 4);
                    return false;
                }
                else
                {
                    //if (numPolygonsClipper == 0) return false;
                    Global.Logger.LogInformation(operationString + " does not match", 2);
                    Global.Logger.LogInformation("clipper time = " + clipTime + "; tvgl time = " + tvglTime, 2);
                    //if (numPolygonsTVGL == numPolygonsClipper)
                    //    Global.Logger.LogInformation("+++ both have {0} polygon(s)", numPolygonsTVGL, numPolygonsClipper);
                    //else 
                    Global.Logger.LogInformation("    --- polygons: TVGL=" + numPolygonsTVGL + "  : Clipper={1} " + numPolygonsClipper, 2);
                    //if (vertsTVGL == vertsClipper)
                    //   Global.Logger.LogInformation("+++ both have {0} vertices(s)", vertsTVGL);
                    //else
                    Global.Logger.LogInformation("    --- verts: TVGL= " + vertsTVGL + "  : Clipper={1} " + vertsClipper, 2);

                    //if (areaTVGL.IsPracticallySame(areaClipper, tolerance))
                    //   Global.Logger.LogInformation("+++ both have area of {0}", areaTVGL);
                    //else
                    Global.Logger.LogInformation("    --- polygons: TVGL=" + areaTVGL + "  : Clipper={1} " + areaClipper, 2);
                    //if (perimeterTVGL.IsPracticallySame(perimeterClipper, tolerance))
                    //    Global.Logger.LogInformation("+++ both have perimeter of {0}", perimeterTVGL);
                    //else
                    Global.Logger.LogInformation("    --- polygons: TVGL=" + perimeterTVGL + "  : Clipper={1} " + perimeterClipper, 2);
                    if (perimeterClipper - perimeterTVGL > 0 && int.IsEvenInteger((int)Math.Round(perimeterClipper - perimeterTVGL)))
                        Global.Logger.LogInformation("<><><><><><><> clipper is connecting separate poly's :", (int)(perimeterClipper - perimeterTVGL) / 2);
                    return true;
                }
            }
        }
...
#else
            sw.Restart();
            var pClipper = BooleanViaClipper(FillRule.Positive, Clipper2Lib.ClipType.ctUnion, new[] { polygonA },
                  new[] { polygonB });
            sw.Stop();
...
#else
            sw.Restart();
            var pClipper = BooleanViaClipper(FillRule.Positive, Clipper2Lib.ClipType.ctUnion, polygons);
            sw.Stop();
            var clipTime = sw.Elapsed;
            ...
            double tolerance = double.NaN)
        {
            ...
            var polygonBList = polygonsB.ToList();
            sw.Restart();
            var pClipper = BooleanViaClipper(FillRule.Positive, Clipper2Lib.ClipType.ctUnion, polygonsA, polygonsB);
            sw.Stop();
            var clipTime = sw.Elapsed;
...
#else
            sw.Restart();
            var pClipper = BooleanViaClipper(FillRule.Positive, Clipper2Lib.ClipType.ctIntersection, new[] { polygonA },
                    new[] { polygonB });
            sw.Stop();
            var clipTime = sw.Elapsed;
            sw.Restart();
        ...
        public static List<Polygon> IntersectPolygons(this IEnumerable<Polygon> polygonsA, IEnumerable<Polygon> polygonsB, PolygonCollection outputAsCollectionType = PolygonCollection.PolygonWithHoles)
        {
            ...
            var polygonBList = polygonsB.ToList();
            sw.Restart();
            var pClipper = BooleanViaClipper(FillRule.Positive, Clipper2Lib.ClipType.ctIntersection, polygonsA, polygonsB);
            sw.Stop();
            var clipTime = sw.Elapsed;
            sw.Restart();
        ...
        public static List<Polygon> IntersectPolygons(this IEnumerable<Polygon> polygons, PolygonCollection outputAsCollectionType = PolygonCollection.PolygonWithHoles)
        {
            ...
            var polygonList = polygons.ToList();
            sw.Restart();
            var pClipper = BooleanViaClipper(FillRule.Positive, Clipper2Lib.ClipType.ctIntersection, polygons);
            sw.Stop();
            var clipTime = sw.Elapsed;
            sw.Restart();
            var result = new List<Polygon>();
...
#else
                    sw.Restart();
                    var pClipper = BooleanViaClipper(FillRule.Positive, Clipper2Lib.ClipType.ctDifference, new[] { minuend },
                        new[] { subtrahend });
                    sw.Stop();
                    var clipTime = sw.Elapsed;
                    sw.Restart();
        ...
        foreach (var polyB in subtrahends)
                    ...
                    var minuendsList = minuends.ToList();
                    sw.Restart();
                    var pClipper = BooleanViaClipper(FillRule.Positive, Clipper2Lib.ClipType.ctDifference, minuends, subtrahends);
                    sw.Stop();
                    var clipTime = sw.Elapsed;
                    sw.Restart();
...
#else
            sw.Restart();
            var pClipper = BooleanViaClipper(FillRule.Positive, Clipper2Lib.ClipType.ctXor, new[] { polygonA }, new[] { polygonB });
            sw.Stop();
            var clipTime = sw.Elapsed;
            sw.Restart();
            var relationship = GetPolygonInteraction(polygonA, polygonB);
