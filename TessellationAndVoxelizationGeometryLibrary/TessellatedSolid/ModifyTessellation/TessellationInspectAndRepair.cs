    ...
    public class TessellationInspectAndRepair
    {
            ...
            out List<TriangleFace> removedFaces, out List<Edge> removedEdges, out List<Vertex> removedVertices)
        {
            ...
            if (buildOptions.FixEdgeDisassociations && OverusedEdges.Count + SingleSidedEdgeData.Count > 0)
            {
                ...
                {
                    Log.Error("Error setting up all faces-edges-vertices associations.", 1);
                }
            }
            if (buildOptions.AutomaticallyRepairNegligibleFaces && InconsistentMatingFacePairs.Any())
            {
                try
                {
                    if (!FlipFacesBasedOnInconsistentEdges())
                        Log.Error("Unable to resolve face all inconsistent faces.", 1);
                }
                catch
                {
                    //Continue
            ...
            if (buildOptions.AutomaticallyRepairNegligibleFaces && buildOptions.PredefineAllEdges)
            {
                try
                {
                    if (!PropagateFixToNegligibleFaces(removedVertices, removedFaces, removedEdges))
                        Log.Information("Unable to flip edges to avoid negligible faces.", 1);
                }
                catch
                {
                    //Continue
            ...
            if (buildOptions.FindNonsmoothEdges)
            {
                ...
                {
                    Log.Information("Unable to find all non-smooth edges.", 1);
                }
            }
            if (buildOptions.CheckModelPostBuild)
            {
                ...
                {
                    Log.Information("Failed final post-build check.", 1);
                }
            }
        }
        #endregion
        ...
        void CheckModelIntegrityPreBuild()
        {
            ...
            {
                ts.Errors = null;
                Log.Information("No errors found.", 3);
            }
        }
        ...
        void CheckModelIntegrityPostBuild()
        {
            if (3 * ts.NumberOfFaces != 2 * ts.NumberOfEdges)
                Log.Information("3 x numFaces = " + 3 * ts.NumberOfFaces + ", 2 x numEdges = " + 2 * ts.NumberOfEdges, 0);
            if (ts.Errors == null)
                Log.Information("No errors were found initially.", 2);
            else
            {
                Log.Information("Errors were found initially.", 2);
            ...
            foreach (var face in ts.Faces)
            {
                foreach (var edge in face.Edges)
                {
                    ...
                    {
                        errors = true;
                        Log.Information("face's edge doesn't reconnect to face", 0);
                    }
                }
                foreach (var vertex in face.Vertices.Where(vertex => !vertex.Faces.Contains(face)))
                {
                    errors = true;
                    Log.Information("face's vertex doesn't reconnect to face", 0);
                }
            }
            ...
            foreach (var edge in ts.Edges)
            {
                ...
                {
                    errors = true;
                    Log.Information("edge's face doesn't reconnect to edge", 0);
                }
                ...
                {
                    errors = true;
                    Log.Information("edge's vertex doesn't reconnect to edge", 0);
                }
            ...
            {
                errors = true;
                Log.Information("there are " + numSingleSidedEdges + " single sided edges", 0);
            }
            ...
            foreach (var vertex in ts.Vertices)
            {
                ...
                {
                    errors = true;
                    Log.Information("vertex's edge doesn't reconnect to vertex", 0);
                }
                foreach (var face in vertex.Faces.Where(face => !face.Vertices.Contains(vertex)))
                {
                    errors = true;
                    Log.Information("vertex's face doesn't reconnect to vertex", 0);
                }
            }
            if (errors)
                Log.Information("The model still contains errors.", 0);
            else if (ts.Errors != null)
            {
                Log.Information("All errors in the model have been fixed.", 1);
                ts.Errors = null;
            }
        }
        ...
        {
            Log.Information("Errors found in model:", 3);
            Log.Information("======================", 3);
            if (tsErrors.ModelHasNegativeVolume)
                Log.Information("==> The model has negative area. The normals of the faces are pointed inward, or this is only a concave surface - not a watertight solid.", 3);
            if (tsErrors.FacesWithNegligibleArea.Count > 0)
                Log.Information("==> " + tsErrors.FacesWithNegligibleArea.Count + " faces with negligible area.", 3);
            if (tsErrors.OverusedEdges.Count > 0)
            {
                Log.Information("==> " + tsErrors.OverusedEdges.Count + " overused edges.", 3);
                Log.Information("    The number of faces per overused edge: " + string.Join(',',
                               tsErrors.OverusedEdges.Select(p => p.Count)), 3);
            }
            if (tsErrors.SingleSidedEdgeData.Count > 0) Log.Information("==> " + tsErrors.SingleSidedEdgeData.Count + " single-sided edges.", 3);
            if (tsErrors.InconsistentMatingFacePairs.Count > 0) Log.Information("==> " + tsErrors.InconsistentMatingFacePairs.Count
                + " edges with opposite-facing faces.", 3);

            if (tsErrors.CorrectFaceEdgeRatio)
                Log.Information("==> While re-connecting faces and edges has lead to errors, there is a likelihood that water-tightness can be acheived."
                    , 3);
            else Log.Information("==> The model is not water-tight. It merely represents a surface, but fixing holes may restore it.", 3);
        }
                    ...
                    out List<Edge> newEdges, out List<TriangleFace> newFaces)
        {
            ...
            {
                Log.Information("Patching hole #" + ++k + " (has " + loop.Count + " edges) in tessellation.", 2);
                ...
                if (winningTask == 0)
                {
                    Log.Information("loop successfully repaired with 3D QuickTriangulate " + triangleFaceList1.Count, 4);
                ...
                {
                    Log.Information("loop successfully repaired with SweepLine 2D Triangulate" + triangleFaceList2.Count, 4);
