    ...
    public partial class ConvexHull3D
{
        ...
        where TFace : IConvexFace3D, new()
    {
        ...
        { // only one extreme point, so the convex hull is a single point
            convexHull = new ConvexHull3D<TVertex, TEdge, TFace>();
        ...
        var faceQueue = new UpdatablePriorityQueue<TFace, double>(simplexFaces.Select(f => (f, f.peakDistance)), new NoEqualSort<double>());
