...
/// </summary>
public static class MinimumSphereClass
{
    public static Sphere MinimumSphere(this IEnumerable<Vector3> pointsInput)
    {
        ...
        {
            var sphere2 = CreateFrom2Points(points[0], points[1]);
            return new Sphere(sphere2.Center, Math.Sqrt(sphere2.Radius));
        }
        else if (numPoints == 3)
        {
            var sphere3 = FirstSphereWith3Points(points[0], points[1], points[2]);
            return new Sphere(sphere3.Center, Math.Sqrt(sphere3.Radius));
        }
        var sphere = FirstSphere(points);
        var startIndex = 4;
        var maxDistSqared = sphere.Radius;
    ...
    private static Sphere FirstSphere(Vector3[] points)
    {
        ...
        if ((points[2] - sphere.Center).LengthSquared() <= sphere.Radius
            && (points[3] - sphere.Center).LengthSquared() <= sphere.Radius)
            return sphere;
        // 0,2 & check 1 & 3
        sphere = CreateFrom2Points(points[0], points[2]);
        if ((points[1] - sphere.Center).LengthSquared() <= sphere.Radius
            && (points[3] - sphere.Center).LengthSquared() <= sphere.Radius)
            return sphere;
        // 0,3 & check 1 & 2
        sphere = CreateFrom2Points(points[0], points[3]);
        if ((points[1] - sphere.Center).LengthSquared() <= sphere.Radius
            && (points[2] - sphere.Center).LengthSquared() <= sphere.Radius)
            return sphere;
        // 1,2 & check 0 & 3
        sphere = CreateFrom2Points(points[1], points[2]);
        if ((points[0] - sphere.Center).LengthSquared() <= sphere.Radius
            && (points[3] - sphere.Center).LengthSquared() <= sphere.Radius)
            return sphere;
        // 1,3 & check 0 & 2
        ...
        if ((points[0] - sphere.Center).LengthSquared() <= sphere.Radius
            && (points[1] - sphere.Center).LengthSquared() <= sphere.Radius)
            return sphere;
        ...
        if ((points[3] - sphere.Center).LengthSquared() <= sphere.Radius)
            minRadiusSqd = sphere.Radius;
        // 0,1,3 & check 2
        var swap3And2 = false;
        var tempSphere = CreateFrom3Points(points[0], points[1], points[3]);
        if ((points[2] - tempSphere.Center).LengthSquared() <= tempSphere.Radius
            && tempSphere.Radius < minRadiusSqd)
        {
            sphere = tempSphere;
            swap3And2 = true;
            minRadiusSqd = sphere.Radius;
        ...
        if ((points[1] - tempSphere.Center).LengthSquared() <= tempSphere.Radius
            && tempSphere.Radius < minRadiusSqd)
        {
            sphere = tempSphere;
            swap3And1 = true;
        ...
        if ((points[0] - tempSphere.Center).LengthSquared() <= tempSphere.Radius
            && tempSphere.Radius < minRadiusSqd)
        {
            sphere = tempSphere;
            swap3And0 = true;
        ...
        if (tempSphere.Radius < minRadiusSqd)
        {
            sphere = tempSphere;
    ...
    private static Sphere FindSphere(Vector3[] points)
    {
        ...
        if ((points[2] - sphere.Center).LengthSquared() <= sphere.Radius
            && (points[3] - sphere.Center).LengthSquared() <= sphere.Radius
            && (points[4] - sphere.Center).LengthSquared() <= sphere.Radius)
            return sphere;
        // 0,2 & check 1, 3 & 4
        sphere = CreateFrom2Points(points[0], points[2]);
        if ((points[1] - sphere.Center).LengthSquared() <= sphere.Radius
        ...
        var tempSphere = CreateFrom3Points(points[0], points[1], points[3]);
        if ((points[2] - tempSphere.Center).LengthSquared() <= tempSphere.Radius
            && (points[4] - tempSphere.Center).LengthSquared() <= tempSphere.Radius
            && tempSphere.Radius < minRadiusSqd)
        {
            sphere = tempSphere;
            swap3And2 = true;
        ...
        if ((points[2] - tempSphere.Center).LengthSquared() <= tempSphere.Radius
            && (points[3] - tempSphere.Center).LengthSquared() <= tempSphere.Radius
            && tempSphere.Radius < minRadiusSqd)
        {
            sphere = tempSphere;
            swap4And2 = true;
        ...
        if ((points[1] - tempSphere.Center).LengthSquared() <= tempSphere.Radius
            && (points[2] - tempSphere.Center).LengthSquared() <= tempSphere.Radius
            && tempSphere.Radius < minRadiusSqd)
        {
            sphere = tempSphere;
            swap12With34 = true;
        ...
        // 0,1,2,3 & check 4
        tempSphere = CreateFrom4Points(points[0], points[1], points[2], points[3]);
        if (!(points[4] - tempSphere.Center).LengthSquared().IsGreaterThanNonNegligible(tempSphere.Radius)
        ...
        if ((points[3] - tempSphere.Center).LengthSquared() <= tempSphere.Radius
            && tempSphere.Radius < minRadiusSqd)
        {
            sphere = tempSphere;
    ...
    private static Sphere CreateFrom4Points(Vector3 p1, Vector3 p2, Vector3 p3, Vector3 p4)
    {
        ...
        // note the sphere returned from this function is really the radius squared!
    ...
    private static Sphere CreateFrom3Points(Vector3 p1, Vector3 p2, Vector3 p3)
    {
        ...
        var center = PointCommonToThreePlanes(planeNormal, planeDist, normal1, dist1, normal2,
            dist2);
        // note the sphere returned from this function is really the radius squared!
        return new Sphere(center, (p1 - center).LengthSquared());
    }
    ...
    private static Vector3 PointCommonToThreePlanes(Vector3 n1, double d1, Vector3 n2, double d2, Vector3 n3,
        double d3)
    {
        var matrixOfNormals = new Matrix3x3(n1.X, n1.Y, n1.Z, n2.X, n2.Y, n2.Z, n3.X, n3.Y, n3.Z );
        var distances = new Vector3(d1, d2, d3);
        return matrixOfNormals.Solve(distances);
    }
}
