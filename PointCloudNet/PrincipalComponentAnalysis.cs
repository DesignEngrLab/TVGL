...
public static class PrincipalComponentAnalysis
{
            ...
            out Vector3 normal)
    {
        ...
        var matrix = new Matrix3x3(xSq, xy, xz, xy, ySq, yz, xz, yz, zSq);
        var rhs = new Vector3(xSum, ySum, zSum);
        var normalArray = matrix.Solve(rhs);
        if (!normalArray.IsNull())
        {
            normal = normalArray.Normalize();
