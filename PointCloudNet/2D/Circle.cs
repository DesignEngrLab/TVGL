...
namespace PointCloud
{
    ...
    public readonly struct Circle
    {
        ...
        public static bool CreateFromPoints(IEnumerable<Vector2> points, out Circle circle, out double error)
        {
            ...
            {
                error = 0.0;
                circle = CreateFrom2Points(p1, p2);
                return true;
            }
            else if (n == 3)
            {
                error = 0.0;
                var success = CreateFrom3Points(p1, p2, p3, out circle);
                return success;
            }
        ...
        public static Circle CreateFrom2Points(Vector2 p1, Vector2 p2)
        {
            return new Circle(new Vector2(0.5 * (p1.X + p2.X), 0.5 * (p1.Y + p2.Y)),
            0.25 * ((p2.X - p1.X) * (p2.X - p1.X) + (p2.Y - p1.Y) * (p2.Y - p1.Y)));
        }

        public static bool CreateFrom3Points(Vector2 p1, Vector2 p2, Vector2 p3, out Circle curve)
        {
            double a = p2.X - p1.X;
            double b = p2.Y - p1.Y;
            double c = p3.X - p1.X;
