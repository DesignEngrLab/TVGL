...
namespace PointCloud
{
    ...
    /// "Updatable" refers to the ability to update an item in the queue. Unlike the
    /// standard version, all elements must be unique. All that is added to the standard
    /// version is three methods: Contains, Remove and UpdatePriority - each of these
    /// takes an element and finds it in the priority queue.
    /// These are based on https://github.com/BlueRaja/High-Speed-Priority-Queue-for-C-Sharp
    /// which is now shown to be slower than the .net version (this is especially true
    /// when running in Debug with a large queue (>5k).
    /// </summary>
    ...
    {
        /// <summary>
        /// For easy deletion of an item on the queue, we keep a separate dictionary to find the index
        /// into the _nodes array.
        /// </summary>
        private Dictionary<TElement, int> _elementIndices;
        /// <summary>
        ...
        /// </param>
        public UpdatablePriorityQueue(IComparer<TPriority>? comparer)
        {
            _nodes = Array.Empty<(TElement, TPriority)>();
            _elementIndices = new Dictionary<TElement, int>();
        ...
        ///  The specified <paramref name="initialCapacity"/> was negative.
        /// </exception>
        public UpdatablePriorityQueue(int initialCapacity, IComparer<TPriority>? comparer)
        {
        ...
        ///  which is generally faster than enqueuing individual elements sequentially.
        /// </remarks>
        public UpdatablePriorityQueue(IEnumerable<(TElement Element, TPriority Priority)> items, IComparer<TPriority>? comparer)
        {
        ...
        /// <summary>
        ///  Removes the minimal element and then immediately adds the specified element with associated priority to the <see cref="UpdatablePriorityQueue{TElement, TPriority}"/>,
        /// </summary>
        /// <param name="element">The element to add to the <see cref="UpdatablePriorityQueue{TElement, TPriority}"/>.</param>
        /// <param name="priority">The priority with which to associate the new element.</param>
        /// <exception cref="InvalidOperationException">The queue is empty.</exception>
        ...
        public void EnqueueRange(IEnumerable<(TElement Element, TPriority Priority)> items)
        {
            ...
            if (_size == 0)
            {
                ...
                if (collection is not null)
                {
                    ...
                    int i = 0;
                    _elementIndices = _nodes.ToDictionary(item => item.Element, item => i++);
                    _size = count;
                }
                else
                {
        ...
        /// <summary>
        /// Determines if the element is in the priority queue.
        /// </summary>
        ...
        /// <param name="element"></param>
        /// <returns></returns>
        public bool Remove(TElement element) =>Remove(element, out _, out _);
        /// <summary>
        ...
        {
            if (capacity < 0) throw new ArgumentOutOfRangeException("A negative capacity was received.", nameof(capacity));
        ...
        private void MoveUpDefaultComparer((TElement Element, TPriority Priority) node, int nodeIndex)
        {
            ...
            while (nodeIndex > 0)
            {
                ...
                if (Comparer<TPriority>.Default.Compare(node.Priority, parent.Priority) < 0)
                {
                    ...
                    _elementIndices[parent.Element] = nodeIndex;
            ...
            _elementIndices[node.Element]=nodeIndex;
        ...
        private void MoveDownDefaultComparer((TElement Element, TPriority Priority) node, int nodeIndex)
        {
            ...
            while ((i = GetFirstChildIndex(nodeIndex)) < size)
            {
                ...
                _elementIndices[minChild.Element] = nodeIndex;
        ...
        internal void UpdatePriority(TElement element, TPriority newPriority)
        {
            Remove(element, out _, out _);
        ...
        public sealed class UnorderedItemsCollection : IReadOnlyCollection<(TElement Element, TPriority Priority)>, ICollection
        {
            ...
            void ICollection.CopyTo(Array array, int index)
            {
                ...
                if (array.Rank != 1)
                {
                    throw new ArgumentException("Array rank must be one (multidimensional arrays are not supported).", nameof(array));
                }
                ...
                {
                    throw new ArgumentException("IncompatibleArrayType", nameof(array));
                }
            }
            ...
            public struct Enumerator : IEnumerator<(TElement Element, TPriority Priority)>
            {
                ...
                {
                    UpdatablePriorityQueue<TElement, TPriority> localQueue = _queue;
