...
namespace PointCloud
{
    public static partial class ConvexHullGJK
    {
        ...
        public static double DistanceBetween<T1, T2>(this IList<T1> cvxHullPoints1, IList<T2> cvxHullPoints2, out Vector3 v)
            where T1 : IConvexVertex3D
            where T2 : IConvexVertex3D
        {
            ...
            v = new Vector3(currentPt1.Coordinates.X - currentPt2.Coordinates.X, currentPt1.Coordinates.Y - currentPt2.Coordinates.Y, currentPt1.Coordinates.Z - currentPt2.Coordinates.Z);
            ...
            do
            {
                ...
                var w = new Vector3(currentPt1.Coordinates.X - currentPt2.Coordinates.X, currentPt1.Coordinates.Y - currentPt2.Coordinates.Y, currentPt1.Coordinates.Z - currentPt2.Coordinates.Z);
        ...
        private static Vector3 projectOnPlane(Vector3 p, Vector3 q, Vector3 r, Vector3 v)
        {
            ...
            return Vector3.Multiply((float)tmp, n);
        }
        ...
        /// <returns></returns>
        private static T FindMaxDotProduct<T>(IList<T> vertices, T datum, Vector3 dir) where T : IConvexVertex3D
        {
            int maxIndex = -1;

            var maxValue = datum.Coordinates.Dot(dir);

            for (int i = 0; i < vertices.Count; ++i)
            {
                var dot = vertices[i].Coordinates.Dot(dir);
                if (dot > maxValue)
                {
                    maxValue = dot;
                    maxIndex = i;
