...
public sealed class Presenter3D : IPresenter3D
{
    ...
    {
        var scene = Scene(); scene.PointSets.Add(Points(points, radius, color ?? new Color(KnownColors.Red))); host.Show(scene);
    }
    public void ShowPointsAndHang(IEnumerable<IEnumerable<Vector3>> pointSets, double radius = 0,
        IEnumerable<Color>? colors = null)
    {
        var scene = Scene(); var palette = colors?.ToList() ?? []; var i = 0; foreach (var set in pointSets) scene.PointSets.Add(Points(set, radius, i < palette.Count ? palette[i] : RandomColor(i++))); host.Show(scene);
    }
    public void ShowAndHangTransparentsAndSolids(IEnumerable<TessellatedSolid> a, IEnumerable<TessellatedSolid> b)
    {
        var scene = Scene(); foreach (var solid in b) AddSolid(scene, solid); foreach (var solid in a) scene.Meshes.Add(Mesh(solid.Faces, Transparent(Rgba(solid.SolidColor)), solid.HasUniformColor, "transparent")); host.Show(scene);
    }
    public void ShowGaussSphereWithIntensity(IEnumerable<Vertex> v, IList<Color> c, Solid s)
    {
        var scene = Scene(); AddSolid(scene, s); var radius = Math.Max(s.XMax - s.XMin, Math.Max(s.YMax - s.YMin, s.ZMax - s.ZMin)) / 2; var center = s.Center; var verts = v.ToList(); for (var i = 0; i < verts.Count; i++) Paths(scene, [[center, center + verts[i].Coordinates * radius]], [false], [5.0], [i < c.Count ? c[i] : new Color(KnownColors.Red)]); host.Show(scene);
    }
    ...
    {
        var scene = Scene(t: title); foreach (var solid in s) AddSolid(scene, solid); Publish(scene, h, t, id);
    }
    ...
    {
        var scene = Scene(t: title); Paths(scene, p, c, t, co); foreach (var solid in s) AddSolid(scene, solid); Publish(scene, h, time, id);
    }
    ...
    {
        var faceGroups = s?.Select(g => g.Select(x => x is TessellatedSolid ts ? ts.Faces : Enumerable.Empty<TriangleFace>())); ShowStepsAndHang(p, pt, faceGroups, st, c, t, co);
    }
    public void ShowStepsAndHang(IEnumerable<IEnumerable<IEnumerable<Vector3>>> p, IEnumerable<IEnumerable<Matrix4x4>> pt,
        IEnumerable<IEnumerable<IEnumerable<TriangleFace>>> f, IEnumerable<IEnumerable<Matrix4x4>> ft, IEnumerable<bool>? c = null,
        IEnumerable<double>? t = null, IEnumerable<Color>? co = null)
    {
        var pathGroups = p?.ToList() ?? []; var faceGroups = f?.ToList() ?? []; var count = Math.Max(pathGroups.SelectMany(g => g).Count(), faceGroups.SelectMany(g => g).Count()); var request = Scene(); for (var index = 0; index < count; index++) { var step = Scene(); foreach (var group in pathGroups) if (index < group.Count()) Paths(step, [group.ElementAt(index)], c, t, co); foreach (var group in faceGroups) if (index < group.Count()) step.Meshes.Add(Mesh(group.ElementAt(index), Rgba32.LightGray, false, "step")); request.Steps.Add(step); } host.Show(request);
    }
    ...
    private static void AddSolid(SceneRequest s, Solid x)
    {
        if (x is TessellatedSolid ts)
            ...
            s.PointSets.Add(new ScenePointSet { Id = $"voxels-{Guid.NewGuid():N}", Radius = Math.Max(1, vs.VoxelSideLength), Color = Rgba(vs.SolidColor), Points = vs.GetExposedVoxels().Select(v => new[] { (float)(v.xIndex * vs.VoxelSideLength + vs.Offset.X), (float)(v.yIndex * vs.VoxelSideLength + vs.Offset.Y), (float)(v.zIndex * vs.VoxelSideLength + vs.Offset.Z) }).ToList() });
    }
    ...
    private static ScenePointSet Points(IEnumerable<Vector3> points, double radius, Color color) => new() { Id = $"points-{Guid.NewGuid():N}", Radius = radius <= 0 ? 1 : radius, Color = Rgba(color), Points = points.Select(p => new[] { (float)p.X, (float)p.Y, (float)p.Z }).ToList() };
    private void Publish(SceneRequest scene, HoldType hold, int time, int id) => host.Publish(new SceneRequest { RequestId = scene.RequestId, Title = scene.Title, Meshes = scene.Meshes, Paths = scene.Paths, PointSets = scene.PointSets, IsBlocking = false, PersistentId = id, HoldType = hold, DisplayIntervalMilliseconds = time });
    private static Rgba32 Transparent(Rgba32 color) => color.A == byte.MaxValue ? color with { A = 89 } : color;
    private static Color RandomColor(int i) => new((byte)((i * 97 + 40) % 220), (byte)((i * 57 + 90) % 220), (byte)((i * 131 + 20) % 220));
}
