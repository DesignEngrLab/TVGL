...
namespace TVGL_Test
{
    ...
    class ClipperTest
    {
        ...
        public static void InnerBoundingCircle()
        {
            ...
            var maxInnerCircle = MinimumEnclosure.MaximumInnerCircle(new List<Path>() { subjectList }, new PointLight(0, 90));
            ...
            maxInnerCircle = MinimumEnclosure.MaximumInnerCircle(new List<Path>() { subjectList }, new PointLight(0, 160));
            Assert.That(maxInnerCircle.Radius.IsPracticallySame(5.0));

            maxInnerCircle = MinimumEnclosure.MaximumInnerCircle(new List<Path>() { subjectList }, new PointLight(0, 165));
            Assert.That(maxInnerCircle.Radius.IsPracticallySame(0.0));

            maxInnerCircle = MinimumEnclosure.MaximumInnerCircle(new List<Path>() { subjectList }, new PointLight(0, 180));
            Assert.That(maxInnerCircle.Radius.IsPracticallySame(0.0));

            maxInnerCircle = MinimumEnclosure.MaximumInnerCircle(new List<Path>() { subjectList }, new PointLight(150, 200));
            Assert.That(maxInnerCircle.Radius.IsPracticallySame(0.0));

            maxInnerCircle = MinimumEnclosure.MaximumInnerCircle(new List<Path>() { subjectList }, new PointLight(155, 200));
            Assert.That(maxInnerCircle.Radius.IsPracticallySame(5.0));

            subjectList.Reverse(); //CW == Hole
            maxInnerCircle = MinimumEnclosure.MaximumInnerCircle(new List<Path>() { subjectList }, new PointLight(0, 90));
            Assert.That(maxInnerCircle.Radius.IsPracticallySame(0.0));
        ...
        public void Difference1()
        {
            ...
            //PolygonOperations automatically applies a scaling factor.
            ...
            solution = PolygonOperations.Union(clipList, subjectList, false);
            //ShowPaths(solution);
            Assert.That(solution.Count, Is.EqualTo(1));


            solution = PolygonOperations.Difference(subjectList, clipList, false);
            //ShowPaths(solution);
            Assert.That(solution.Count, Is.EqualTo(2));
            //Check if they are pftPositive polygons
            Assert.That(MiscFunctions.AreaOfPolygon(solution[0]) > 0.0);
            Assert.That(MiscFunctions.AreaOfPolygon(solution[1]) > 0.0);

            solution = PolygonOperations.Intersection(subjectList, clipList, false);
            //ShowPaths(solution);
            Assert.That(solution.Count, Is.EqualTo(1));

            solution = PolygonOperations.Xor(subjectList, clipList, false);
            //ShowPaths(solution);
        ...
        public void Horz1()
        {
            ...
            solution = new Paths();
            solution = PolygonOperations.Intersection(subject, clip, false);
            var result = solution.Count > 0;
            //ShowPaths(solution);
            Assert.That(result, Is.True);
            Assert.That(solution.Count, Is.EqualTo(1));
            ...
            //Even odd if you want to output the bottom right corner as a positive polygon. 
            //By default, it would actually be removed since it is negative.
            solution = PolygonOperations.Difference(subject, clip, false, PolygonFillType.EvenOdd);
            result = solution.Count > 0;
            //ShowPaths(solution);
            Assert.That(result, Is.True);
            Assert.That(solution.Count, Is.EqualTo(2));

            //now try the default case, where the bottom right is removed
            solution = PolygonOperations.Difference(subject, clip);
            result = solution.Count > 0;
            //ShowPaths(solution);
            Assert.That(result, Is.True);
            Assert.That(solution.Count, Is.EqualTo(1));
            ...
            //Even odd if you want to output the bottom right corner as a positive polygon. 
            //By default, it would actually be removed since it is negative.
            solution = PolygonOperations.Xor(subject, clip, false, PolygonFillType.EvenOdd);
            result = solution.Count > 0;
            //ShowPaths(solution);
            Assert.That(result, Is.True);
            Assert.That(solution.Count, Is.EqualTo(4));
            ...
            //now try the default case, where the bottom right is removed
            solution = PolygonOperations.Xor(subject, clip);
            result = solution.Count > 0;
            //ShowPaths(solution);
            Assert.That(result, Is.True);
        ...
        //    public void Horz2()
        //    {
        //        PolyFillType fillMethod = PolyFillType.pftEvenOdd;
        ...
        //        clipper.AddPaths(subject, PolyType.ptSubject, true);
        //        clipper.AddPaths(clip, PolyType.ptClip, true);
        //        bool result = clipper.Execute(ClipType.ctIntersection, solution, fillMethod, fillMethod);
        ...
        //        clipper.AddPaths(subject, PolyType.ptSubject, true);
        //        bool result = clipper.Execute(ClipType.ctUnion, solution, fillMethod, fillMethod);
        ...
        //    public void Horz5()
        //    {
        //        PolyFillType fillMethod = PolyFillType.pftNonZero;
        ...
        //        clipper.AddPaths(subject, PolyType.ptSubject, true);
        //        clipper.AddPaths(clip, PolyType.ptClip, true);
        //        bool result = clipper.Execute(ClipType.ctDifference, solution, fillMethod, fillMethod);
        ...
        //        clipper.AddPaths(subject2, PolyType.ptSubject, false);
        //    }
        ...
        //        clipper.AddPaths(subject2, PolyType.ptSubject, false);
        //        clipper.AddPaths(clip, PolyType.ptClip, true);
        //        bool result = clipper.Execute(ClipType.ctIntersection, polytree, fillMethod, fillMethod);
        ...
        //        clipperOffset.AddPaths(subject, JoinType.jtRound, EndType.etClosedPolygon);
