...
{
    public class Presenter3D_SoftwareRender : IPresenter3D
    {
    ...
    HoldType holdType = HoldType.Immediate, int timetoShow = -1, int id = -1)
        ...
        /// <summary>
        /// A lazily-created, process-wide <see cref="DefaultEffectsManager"/> backed by the
        /// Direct3D 11 WARP software rasteriser.  Assign this to the <c>EffectsManager</c>
        /// property of every <c>Viewport3DX</c> in your window code-behind, e.g.:
        /// <code>
        ///     MyViewport.EffectsManager = Presenter3D.SoftwareEffectsManager;
        /// </code>
        /// </summary>
        public static DefaultEffectsManager SoftwareEffectsManager
            => _softwareEffectsManager ??= CreateSoftwareEffectsManager();
        private static DefaultEffectsManager _softwareEffectsManager;

        /// <summary>
        /// Creates a HelixToolkit <see cref="DefaultEffectsManager"/> that is backed by the
        /// Direct3D 11 WARP software rasteriser instead of a physical GPU.
        /// Pass the returned instance to your viewport's <c>EffectsManager</c> property
        /// before the window is shown, e.g.:
        /// <code>
        ///     viewport.EffectsManager = Presenter3D.CreateSoftwareEffectsManager();
        /// </code>
        /// </summary>
        /// <returns>
        /// A <see cref="DefaultEffectsManager"/> running on the WARP adapter, or
        /// <c>null</c> if WARP is not available on this machine.
        /// </returns>
        public static DefaultEffectsManager CreateSoftwareEffectsManager()
        {
            try
            {
                // Ask DXGI for the WARP adapter (index 0 of software adapters).
                // SharpDX.DXGI is already a transitive dependency of HelixToolkit.Wpf.SharpDX.
                using var factory = new SharpDX.DXGI.Factory1();
                // WARP is exposed as a software adapter via CreateSoftwareAdapter / QueryInterface.
                // The simplest cross-version approach is to enumerate adapters and pick the one
                // whose description contains "Microsoft Basic Render" or "WARP", which is the
                // software rasteriser registered by Windows.
                SharpDX.DXGI.Adapter1 warpAdapter = null;
                for (int i = 0; i < factory.GetAdapterCount1(); i++)
                {
                    var candidate = factory.GetAdapter1(i);
                    var desc = candidate.Description1.Description;
                    if (desc.IndexOf("WARP", StringComparison.OrdinalIgnoreCase) >= 0 ||
                        desc.IndexOf("Basic Render", StringComparison.OrdinalIgnoreCase) >= 0)
                    {
                        warpAdapter = candidate;
                        break;
                    }
                    candidate.Dispose();
                }
                ...
                if (warpAdapter == null)
                {
                    // Fallback: use D3D11CreateDevice with DriverType.Warp directly.
                    // DefaultEffectsManager(int adapterIndex) where -1 means WARP on some builds;
                    // instead we rely on the adapter-based constructor available in recent HelixToolkit.
                    Debug.WriteLine("[Presenter3D] WARP adapter not found via enumeration; falling back to default software path.");
                    return new DefaultEffectsManager(-1); // -1 selects WARP in HelixToolkit >= 2.13
                }

                int adapterIndex = 0;
                // Re-enumerate to find the numeric index expected by DefaultEffectsManager(int).
                for (int i = 0; i < factory.GetAdapterCount1(); i++)
                {
                    using var a = factory.GetAdapter1(i);
                    if (a.NativePointer == warpAdapter.NativePointer)
                    {
                        adapterIndex = i;
                        break;
                    }
                ...
                // DefaultEffectsManager(int adapterIndex) selects the DXGI adapter at that index.
                return new DefaultEffectsManager(adapterIndex);
            }
            catch (Exception ex)
            {
                Debug.WriteLine($"[Presenter3D] Could not create WARP EffectsManager: {ex.Message}");
                return null;
            }
        }
    }
